...
Extract names of low mean and low detected genes:

```{r}
low <- rowData(sce)$low_mean | rowData(sce)$low_detected
```
...
var <- c("sum", "detected", "subsets_MT_percent", "altexps_ERCC_percent", "subsets_EGFP_percent", "phenotype")
...
min <- ifelse(ncol(sce) < 100, ncol(sce), 100)
...
Calculate logNormCounts assay and PCA, TSNE, and UMAP reduced dimensions:
